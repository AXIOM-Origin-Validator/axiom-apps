...
struct ReleaseInfo: Codable, Equatable {
    let version: String
    let coreId: String
    // `dmg` + `sha256` are OPTIONAL: `products` in releases.json also
    // carries non-DMG entries (e.g. `webclient`, which has html_sha256/
    // zip_sha256 and no dmg). The manifest decodes the WHOLE products
    // dict into [String: ReleaseInfo], so a sibling entry that lacks
    // these fields must not fail-decode the entire manifest (it did —
    // "the data couldn't be read because it is missing"). The pinned
    // DMG products this app actually consumes (axiomwallet / unclesam)
    // always populate both; the download path guards on them.
    let dmg: String?
    let url: String?
    let sha256: String?
    let notesUrl: String?
...
/// releases.json is download artifacts only; network identity lives in
/// `worldline.json` (below).
struct ReleaseManifest: Codable {
    let schema: Int
    let products: [String: ReleaseInfo]
}

/// `seeds/worldline.json` — the network's current parameters: which Core
/// defines the live worldline, and the *suggested* L$ digit_version
/// (with the date it took effect). Advisory + best-effort: if this file
/// is unreachable it is treated as simply absent — the app never blocks
/// and shows only a quiet line in About.
...
enum ReleaseUpdate {
    ...
    ///
    /// `AXIOM_RELEASES_URL` overrides it (debug / QA): point it at a
    /// local `file://` manifest or a staging URL to exercise the update
    /// flow on demand — e.g. a manifest with a bumped version so this
    /// build sees an "available" update and the banner + Download path
    /// can be walked end to end. Empty / unset → the published feed.
    ...
    /// digit_version. Lives in axiom-dist/seeds/ next to the seed lists.
    /// Best-effort: unreachable is treated as absent, never an error.
    /// `AXIOM_WORLDLINE_URL` overrides for testing.
    ...
    /// bundle Info.plist. This is the value that matches the DMG/release
    /// identity (`axiomwallet-<coreid8>-<version>.dmg`) and the manifest
    /// `version` field. NOT `sdk_build_version()` — that returns the Rust
    /// CRATE version (workspace 3.2.0), a different namespace, so
    /// comparing the manifest's app version against it never registers as
    /// newer and the optional-update path would silently never fire.
    ...
    static func verdict(
        ...
        // Mandatory: the live worldline's Core (from worldline.json, the
        // authoritative network identity) differs from this build's baked
        // Core. Skipped when either side is empty — a dev build with no
        // baked CoreID, or worldline.json unreachable (best-effort).
