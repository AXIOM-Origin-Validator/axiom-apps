...
final class ReleaseUpdateWatcher: ObservableObject {
    ...
    /// When the last check completed (success or failure).
    ...
    /// Hard-lock signal: published CoreID differs from this build's — the
    /// network's canonical Core rotated and this client is rejected at the
    /// CoreID gate. Transacting against a mismatched Core would diverge
    /// this wallet from the network (YP §23.10 / §16.8.3).
    ...
    func check() async {
        ...
        // Best-effort worldline params (current Core); failure ⇒ absent.
    ...
    /// Fetch `worldline.json`; nil on any failure (best-effort).
    ...
    /// /Applications. Verifies the sha256 against the manifest.
    func downloadAndReveal() async {
        ...
        do {
            ...
            // a constant stable filename (UNCLESam.dmg), so the CoreID
